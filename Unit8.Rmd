...
This is very interesting, since based on the qq plot we expected overdispersion, but the calculated ratio is less than 1, suggesting underdispersion. **This is a very important learning opportunity for us!** We found something that probably means we have a misconception, and this can be very revealing to resolve.

This discrepancy can occur because the qq-plot is sensitive to, and shows us, extreme residuals, while the deviance ratio summarizes overall fit. The discrepancy therefore suggest that for some parts of the residual distribution, the model fits poorly (extreme residuals, overdispersion), while for other parts the model fits better than would be expected (underdispersion)! Cool! So the two methods concern different features of the residuals.

Addressing this issue in the model / data combination would need modelling of greater sophistication to resolve. Let us continue with the current model and see how well it performs.
