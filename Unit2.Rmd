...
## Why ggplot2? It's so tricky!

There are lots of ways to makes graphs in R. There are even lots of ways to make the same graph. Prof. Petchey promotes the use of the ggplot function in the ggplot2 package, so you will see this being used again and again and again during the practicals and elsewhere. You might find ggplot tricky at first (Prof. Petchey did -- really -- but maybe that was because he already knew other methods). But don't worry, Prof. Petchey now knows what he's doing with ggplot and has infinite patience explaining how it works :)

You won't get penalised for using other methods for making graphs. Its just that it will be a bit harder for us to help you, since we'll first have to think about how to do it that way, rather than just how to make it work with ggplot (or qplot).

[Some reasons for using ggplot are here.](http://varianceexplained.org/r/teach_ggplot2_to_beginners/)

Another reason is that with ggplot you can do just about anything worth doing. Have a look at the [ggplot2 book online version](https://ggplot2-book.org/). It really is pretty amazing.
